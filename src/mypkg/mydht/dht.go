...
func NewDHTNode(port, numTargetPeers int, storeEnabled bool) (node *DHT, err error) {
	...
	l4g.Warn(node.routingTable.addresses)
	return
}
...
func (d *DHT) processPacket(p packetType) {
	...
	case r.Y == "r":
		...
		if query, ok := node.pendingQueries[r.T]; ok {
			...
			case "get_peers":
				//l4g.Warn(EncodeInfoHash(query.ih))
				d.processGetPeerResults(node, r)

			case "find_node":
				d.processFindNodeResults(node, r)
				//l4g.Warn("DHT: Received find_node reply")
...
func (d *DHT) replyAnnouncePeer(addr *net.UDPAddr, r responseType) {
	l4g.Warn("AnnouncePeer:", EncodeInfoHash(InfoHash(r.A.InfoHash)))

	go WriteLog("announce"+date+".txt", EncodeInfoHash(InfoHash(r.A.InfoHash)))
...
func (d *DHT) replyGetPeers(addr *net.UDPAddr, r responseType) {
	l4g.Warn("GetPeers", EncodeInfoHash(InfoHash(r.A.InfoHash)))

	go WriteLog("getpeer"+date+".txt", EncodeInfoHash(InfoHash(r.A.InfoHash)))
...
func WriteLog(fileName string, bewrite string) {
	...
	//logger.Println(bewrite)
	logfile.WriteString(bewrite + "\r\n")
